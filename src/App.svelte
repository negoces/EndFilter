<script lang="ts">
    ...
    let termCount = $derived(
        termOptions.base.length +
            termOptions.extra.length +
            termOptions.skill.length,
    );
                ...
                if (!cancelled) {
                    error =
                        caught instanceof Error
                            ? caught.message
                            : "未能读取本地站点数据，请检查 public/data/sites.json。";
                    sites = [];
                }
            } finally {
                if (!cancelled) {
    ...
    <div class="mx-auto grid w-full max-w-7xl gap-4">
        <section class="grid gap-3 text-center sm:text-left">
            ...
            <div class="grid gap-2 sm:grid-cols-3">
                <div
                    class="rounded-lg border border-border bg-card px-3 py-2 text-card-foreground"
                >
                    <p class="text-xs text-muted-foreground">数据源</p>
        ...
        <div
            class="grid gap-4 lg:grid-cols-[360px_minmax(0,1fr)] lg:items-start"
        >
